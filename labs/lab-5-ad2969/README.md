...
In this task you will add a custom Avalon memory-mapped slave that **continuously** counts the clock cycles since the system was reset. The counter will be a 64-bit-wide unsigned integer. Because we will use a 32-bit Avalon interconnect, we will have to split reading the value into two parts: reading address offset 0 will return bits 31..0 and reading address offset 1 will return bits 63..32. A skeleton is provided in `counter.sv`.
